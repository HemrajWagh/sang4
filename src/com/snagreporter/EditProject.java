...
import com.snagreporter.EditBuilding.UpdateDataToWeb;
import com.snagreporter.EditBuilding.UploadImageToweb;
...
public class EditProject extends Activity
{
	...
	EditText txtPrjName,txtNoOfbld,txtLoc,txtAdd1,txtAdd2,txtPin,txtCity,txtState,txtBldrName;
	Bitmap myBmp;
	ImageView myImg;
    ...
    public void onCreate(Bundle savedInstanceState)
    {
        ...
        menuhandler=new MenuHandler(EditProject.this);
        ...
        mProgressDialog2 = new ProgressDialog(EditProject.this);
        currentproject=(ProjectMaster)getIntent().getExtras().get("project");
        txtBldrName=(EditText)findViewById(R.id.editPrj_blrname);
        txtPrjName=(EditText)findViewById(R.id.editPrj_prjname);
        txtAdd1=(EditText)findViewById(R.id.editPrj_addrss1);
        txtAdd2=(EditText)findViewById(R.id.editPrj_addrss2);
        txtCity=(EditText)findViewById(R.id.editPrj_city);
        txtLoc=(EditText)findViewById(R.id.editPrj_loc);
        txtPin=(EditText)findViewById(R.id.editPrj_pin);
        txtState=(EditText)findViewById(R.id.editPrj_state);
        myImg=(ImageView)findViewById(R.id.editPrj_imgAdd);
        txtBldrName.setText(currentproject.getBuilderName());
       // objPrj=currentproject;
        populatedata();
	...
	public void MenuRoomsheetClick(View v){
		HideTopMenu();
		//menuhandler.MenuRoomsheetClick(c);
	}
	...
	{
		txtPrjName.setText(currentproject.getProjectName());
		if(currentproject.getLocation()!=null && currentproject.getLocation().length()>0)
		{
			txtLoc.setText(currentproject.getLocation());
			Log.d("location", ""+currentproject.getLocation());
		}
		else
		{
			txtLoc.setText("");
		}
		if(currentproject.getAddress1()!=null && currentproject.getAddress1().length()>0)
		{
			txtAdd1.setText(currentproject.getAddress1());
		}
		else
		{
			txtAdd1.setText("");
		}
		if(currentproject.getAddress2()!=null && currentproject.getAddress2().length()>0)
		{
			txtAdd2.setText(currentproject.getAddress2());
		}
		else
		{
			txtAdd2.setText("");
		}
		if(currentproject.getCity()!=null && currentproject.getCity().length()>0)
		{
			txtCity.setText(currentproject.getCity());
		}
		else
		{
			txtCity.setText("");
		}
		if(currentproject.getPincode()!=null && currentproject.getPincode().length()>0)
		{
			txtPin.setText(currentproject.getPincode());
		}
		else
		{
			txtPin.setText("");
		}
		if(currentproject.getState()!=null && currentproject.getState().length()>0)
		{
			txtState.setText(currentproject.getState());
		}
		else
		{
			txtState.setText("");
		}
		if(currentproject.getImageName()!=null && currentproject.getImageName().length()>0)
		{
			Bitmap bmp;
			oldImageName=currentproject.getImageName();
			
			String filepath=Environment.getExternalStorageDirectory()+"/SnagReporter/Pictures/"+oldImageName;
	...
	public void SubmitClick(View v)
	{
		if(istextValid())
		{
			currentproject.setProjectName(txtPrjName.getText().toString());
			currentproject.setAddress1(txtAdd1.getText().toString());
			if(txtAdd2.getText()!=null && txtAdd2.getText().toString().length()>0)
			{
				currentproject.setAddress2(txtAdd2.getText().toString());
			}
			else
			{
				currentproject.setAddress2("");
			}
			currentproject.setCity(txtCity.getText().toString());
			currentproject.setLocation(txtLoc.getText().toString());
			currentproject.setPincode(txtPin.getText().toString());
			currentproject.setState(txtState.getText().toString());
			if(imageName!=null && imageName.length()>0)
			{
				currentproject.setImageName(imageName);
			}
			
			UpdateDataToWeb task=new UpdateDataToWeb();
			task.execute(10);	
			finish();
		}
		else
...
//	 		 	     outStream.close();
	 ...
	 protected class UpdateDataToWeb extends AsyncTask<Integer , Integer, Void> 
	 {
		 String output = "";
		 String value="";
		 ProgressDialog mProgressDialog = new ProgressDialog(EditProject.this);
		 @Override
		protected void onPreExecute() 
		 {
			if(isOnline){
		...
		protected Void doInBackground(Integer... params)
		{
			 ...
			 String METHOD_NAME = "UpdateDataToTheDataBase";
	    		...
	    		try {
	    		    ...
	    		    String Values=""+currentproject.getProjectName()+"~"+currentproject.getLocation()+"~"+currentproject.getAddress1()+"~"+currentproject.getAddress2()+"~"+currentproject.getPincode()+"~"+currentproject.getCity()+"~"+currentproject.getState()+"~"+currentproject.getImageName();
	    		    String TableName="ProjectMaster";
	    		    String KeyColName="ID";
	    		    String ColDataType="String";
	    		    String ColValue=""+currentproject.getID();
		...
		protected void onPostExecute(Void result) 
		{
			 currentproject.setStatusForUpload("Modified");
			 ...
			 {
				 currentproject.setisDataSyncToWeb(true);
				 FMDBDatabaseAccess fdb=new FMDBDatabaseAccess(EditProject.this);
				 fdb.insertORUpdateProject(currentproject);
				 if(imageName!=null && imageName.length()>0 && !(imageName.equalsIgnoreCase(oldImageName)))
				 {
			 ...
			 {
				 new AlertDialog.Builder(EditProject.this)
				 ...
				 currentproject.setisDataSyncToWeb(false);
				 FMDBDatabaseAccess fdb=new FMDBDatabaseAccess(EditProject.this);
				 fdb.insertORUpdateProject(currentproject);
				// finish();
			 }
	 ...
	 public void ChangeStatusClick(View v){
	    	...
	    	public void ShowChartClick(View v){
	    		try{	
	    			Intent i=new Intent(EditProject.this,com.snagreporter.GraphWebView.class);
	            	startActivity(i);
	    		}
	    		catch(Exception e){
	    			Log.d("Error=", ""+e.getMessage());
	    	...
	    	public void SyncData(){
	        	...
	        	//task.execute(10);
	    		ParseSyncData parser=new ParseSyncData(EditProject.this);
	    		parser.start();
	        }
	        protected class StartSyncProgress extends AsyncTask<Integer , Integer, Void> {
	        	...
	        	boolean result=false;
	                ...
	                protected void onPreExecute() {  
	                	...
	                	FMDBDatabaseAccess db=new FMDBDatabaseAccess(EditProject.this);
