...
import com.snagreporter.BuildingListPage.StartSyncProgress;
import com.snagreporter.DefectListPage.DownloadDataFromWeb;
...
public class FloorListPage extends Activity
{
	ListView list;
	ListView list_building;
	...
	String ActiveSnag,ActiveTrade,ActiveFloor,perComplete;
    @Override
    public void onCreate(Bundle savedInstanceState)
    {
        ...
        setContentView(R.layout.floor_list_page);
        ...
        menuhandler=new MenuHandler(FloorListPage.this);
        ...
        mProgressDialog2 = new ProgressDialog(FloorListPage.this);
        ...
        list=(ListView)findViewById(R.id.android_list_floors);
        list_building=(ListView)findViewById(R.id.android_list_floor_bldg);
  ...
  //@@@@@@@MenuHandlers
	...
	public void MenuGraphClick(View v){
		HideTopMenu();
		menuhandler.MenuGraphClick("floor",CurrentBuilding);
	}
    ...
    public void RefreshClick(View v){
		try{
			...
			task.execute(10);
				...
				new AlertDialog.Builder(FloorListPage.this)
	...
	public void onResume(){
        ...
        list_building.setDivider(getResources().getDrawable(R.color.transparent));
        list_building.setDividerHeight(1);
       ...
       list_building.setAdapter(adapterParent);
       
       list_building.setOnItemClickListener(new OnItemClickListener() {
       ...
       //items.add(new Header(null, "FLOORS LIST",false));
       if(LoginType.equals(getResources().getString(R.string.strInspector).toString()))
    	   items.add(new HeaderWithAdd(null, "FLOORS LIST", "Add New Floor",true));
       else
    	   items.add(new HeaderWithAdd(null, "FLOORS LIST", "Add New Floor",false));
       
       
       grid=(GridView)findViewById(R.id.floor_gridview);
     ...
     if(keycode == KeyEvent.KEYCODE_BACK){
    	 try{
    		 ...
    		 SharedPreferences SP = getSharedPreferences("AppDelegate",MODE_PRIVATE);
				...
				db.setCurrentPositionInProject(CurrentProject.getID(), "Project",SP.getString("RegUserID", ""));
	    		if(CameFromStartPage){
	    			Intent i=new Intent(FloorListPage.this,com.snagreporter.BuildingListPage.class);
    ...
    public void populateFloors()
    {
    	try{
    		...
    		arrFloor=null;
    		arrFloor=obj.getFloorsWithSngCnt(CurrentBuilding);//getFloors(CurrentBuilding);
    ...
    protected class DownloadDataFromWeb extends AsyncTask<Integer , Integer, Void> {
    	ProgressDialog mProgressDialog = new ProgressDialog(FloorListPage.this);
    	JSONObject jObject;
        @Override
        	...
        	try
    		{
    			try{
    	    		...
    	    		try {
    	    		    ...
    	    		    request.addProperty("_strTableName","FloorMaster");
			...
			//stuff that updates ui
			    	 try{
						...
						if(arr!=null){
			    		 ...
			    		 FMDBDatabaseAccess obj=new FMDBDatabaseAccess(FloorListPage.this);
			    		 arrFloor=obj.getFloorsWithSngCnt(CurrentBuilding);//getFloors(CurrentBuilding);
			    		 ContinueProcess();
			    	 }
			    	 catch(Exception e){
    ...
    public void ContinueProcess(){
    	...
    	if(arrFloor!=null && arrFloor.length>0){
			for(int i=0;i<arrFloor.length;i++){
				//items.add(new ListItem(null, ""+arrFloor[i].getFloor()));
				try{
				String FilePath=Environment.getExternalStorageDirectory()+"/SnagReporter/Pictures/"+arrFloor[i].getImageName();
				Bitmap bp=BitmapFactory.decodeFile(FilePath);
				Grid_items.add(new GridViewListItem(null, ""+arrFloor[i].getFloor(),2,bp,arrFloor[i].SnagCount,arrFloor[i].isInspStarted));
...
//				bp=null;
					...
					Log.d("OOM Floorlistpage index="+i, oom.getMessage());
				}
			}
		}
		...
		if(arrFloor==null || arrFloor.length==0){
			//items.add(new ListItemBlank(null, "No Floors found"));
    ...
    public void relaodList(){
        ...
        grid.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> arg0, View arg1, int position,
				...
				if( arrFloor!=null && arrFloor.length>0 && (position)<=arrFloor.length){
					if(arrFloor[position].FloorType!=null && arrFloor[position].FloorType.equalsIgnoreCase("EXTERNAL AREA"))
					{
						Intent itt=new Intent(FloorListPage.this,com.snagreporter.DefectListPage.class);
						itt.putExtra("Floor", arrFloor[position]);
						itt.putExtra("Project",CurrentProject);
						itt.putExtra("Building",CurrentBuilding);
						itt.putExtra("isFromFloorArea",true);
						startActivityForResult(itt,10001);
					}
					else
					{
						Intent i=new Intent(FloorListPage.this,com.snagreporter.ApartmentListPage.class);
						i.putExtra("Floor", arrFloor[position]);
						i.putExtra("Project", CurrentProject);
						i.putExtra("Building", CurrentBuilding);
						SharedPreferences SP = getSharedPreferences("AppDelegate",MODE_PRIVATE);
						FMDBDatabaseAccess db=new FMDBDatabaseAccess(getApplicationContext());
						String str=arrFloor[position].getProjectID()+"~"+arrFloor[position].getBuildingID()+"~"+arrFloor[position].getID();
						db.setCurrentPositionInProject(str, "Floor",SP.getString("RegUserID", ""));
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent imageReturnedIntent) 
    {
    	 ...
    	 if(requestCode==101 && resultCode==101 && imageReturnedIntent!=null){
      		...
      		menuhandler.SaveAttachment(CurrentProject.getID(),CurrentBuilding.getID(), "", "", "", "", "", "", FileName, FilePath);
    	...
    	if(resultCode==10001){
    		...
    		finish();
    			...
    			FMDBDatabaseAccess db=new FMDBDatabaseAccess(FloorListPage.this);
                SharedPreferences SP = getSharedPreferences("AppDelegate",MODE_PRIVATE);
    			db.performLogout(SP.getString("RegUserID", ""));
    			Intent i=new Intent(FloorListPage.this,com.snagreporter.Login_page.class);
    ...
    public void populateCurrentBuilding()
    {
    	try{
    		...
    		FMDBDatabaseAccess fmdb=new FMDBDatabaseAccess(FloorListPage.this);
    		
    		TradeAptAreaMaster objTrade=fmdb.getActiveData(CurrentProject.getID(),CurrentBuilding.getID(),"","","floorlist");
    		
    		int actvsn=fmdb.getActiveSnag(CurrentProject.getID(),CurrentBuilding.getID(),"","","floorlist");
    		//@@new
    		//newend
    		ActiveSnag=""+actvsn;
    		if(LoginType.equals(getResources().getString(R.string.strInspector).toString()))
    				itemsParent.add(new Header(null, "BUILDING",true,false,false,true));
    		else
    			itemsParent.add(new Header(null, "BUILDING",true,false,false,false));
    				
    		        itemsParent.add(new ActiveFields(null, ""+CurrentProject.getProjectName()+"  -  "+CurrentBuilding.getBuildingName(),false,""));
    		        //itemsParent.add(new ListItemParent(null, "Project",""+CurrentProject.getProjectName()));
    				//itemsParent.add(new ListItemParent(null, "Building",""+CurrentBuilding.getBuildingName()));
    				//itemsParent.add(new ListItemParent(null, "ActiveSnag",ActiveSnag));
					
    				if(objTrade!=null)
    				{
    					ActiveTrade=objTrade.TradeType;
    		    		FloorMaster objFloor=fmdb.getFloorWithID(objTrade.ProjectID,objTrade.BuildingID,objTrade.FloorID);
    		    		ActiveFloor=objFloor.getFloor();
    					...
    					//itemsParent.add(new ActiveFields2(null,""+ActiveFloor+"  -  "+ActiveTrade,false,""));
    		    		//oldend
    					
    					//@@@@new
    					
    					itemsParent.add(new ActiveFields2(null, "Snags "+ActiveSnag,false,""));
    					itemsParent.add(new ActiveFields2(null,""+ActiveFloor+"  -  "+ActiveTrade,true,"Completed "+perComplete+"%"));
    					...
    					//itemsParent.add(new ListItemParent(null, "ActiveFloor",ActiveFloor));
    				}
    ...
    public void parseData(JSONObject obj){
    	try{
    		...
    		int NoOfApartments=obj.getInt("NoOfApartments");
    		...
    		FloorMaster flr=new FloorMaster();
    		flr.setID(ID);
    		flr.setFloor(Floor);
    		flr.setNoOfApartments(NoOfApartments);
    		flr.setBuildingID(BuildingID);
    		flr.setBuildingName(BuildingName);
    		flr.setProjectID(ProjectID);
    		flr.setProjectName(ProjectName);
    		
    		String ImageName=obj.getString("ImageName");
    		if(ImageName!=null && ImageName.length()!=0){
    			downloadImage(ImageName);
    		}
    		flr.setImageName(ImageName);
    		
    		FMDBDatabaseAccess db=new FMDBDatabaseAccess(getApplicationContext());
    		db.insertORUpdateFloor(flr);
    	}
    	catch(Exception e){
    		Log.d("Error=", ""+e.getMessage());
    	}
    ...
    {
    	try{
    	Intent i=new Intent(FloorListPage.this,com.snagreporter.AddNewFloor.class);
    	i.putExtra("Project", CurrentProject);
    	i.putExtra("Building",CurrentBuilding);
    	startActivity(i);
    	}
    ...
    {
    	try{
    	Intent i=new Intent(FloorListPage.this,com.snagreporter.EditBuilding.class);
    	i.putExtra("project",CurrentProject);
    	i.putExtra("building",CurrentBuilding);
    	startActivity(i);
    	}
...
//		};
  ...
  private void openChart(){
  	...
  	mFloors=new String[arrFloor.length];
  	for(int i=0;i<arrFloor.length;i++){
  		mFloors[i]=arrFloor[i].getFloor();
  	}
  	
  	int[] x=new int[arrFloor.length];
  	for(int i=0;i<arrFloor.length;i++){
  		x[i]=i;
  	}
  	FMDBDatabaseAccess db=new FMDBDatabaseAccess(FloorListPage.this);
  	
  	int[] resolved = new int[arrFloor.length];//{ 2000,2500,2700,3000,2800,3500,3700,3800};
  	for(int i=0;i<arrFloor.length;i++){
  		resolved[i]=db.getResolvedSnagsByFloor(CurrentProject.getID(), CurrentBuilding.getID(), arrFloor[i].getID(), id);
  	}
  	
  	int[] expense = new int[arrFloor.length];//{2200, 2700, 2900, 2800, 2600, 3000, 3300, 3400 };
  			for(int i=0;i<arrFloor.length;i++){
  				expense[i]=db.getUnresolvedSnagsByFloor(CurrentProject.getID(), CurrentBuilding.getID(), arrFloor[i].getID(), id);
  	    	}
  	...
  	multiRenderer.setChartTitle(""+CurrentProject.getProjectName()+" - "+CurrentBuilding.getBuildingName());
  	multiRenderer.setXTitle("Floors");
  	multiRenderer.setYTitle("Number of Snags");
  	multiRenderer.setZoomButtonsVisible(true);    	    	
  	for(int i=0; i< x.length;i++){
  		multiRenderer.addXTextLabel(i, mFloors[i]);    		
  	}    	
    ...
    public boolean onOptionsItemSelected(MenuItem item)
    {
        ...
        case R.id.menu_showlistinWeb:
        	Intent i=new Intent(FloorListPage.this,com.snagreporter.GraphWebView.class);
        	startActivity(i);
            return true;
...
//    				finish();
    ...
    public void SyncData(){
    	...
    	//task.execute(10);
    	ParseSyncData parser=new ParseSyncData(FloorListPage.this);
		parser.start();
    }
...
//            		mProgressDialog2.dismiss();
    ...
    public void ChangeStatusClick(View v){
    	...
    	public void openChart2(){
    		try{
    			Intent i=new Intent(FloorListPage.this,com.snagreporter.BarChartActivity.class);
